...
from books.models import DISTRICT_CHOICES
from sellers.models import PAYMENT_OPTIONS
...
class myRegistrationForm(RegistrationFormUniqueEmail):
    """
    Custom Subclass of ``RegistrationFormUniqueEmail`` which enforces uniqueness of
    ...
    hide_cellular = forms.BooleanField(widget=forms.CheckboxInput(attrs=attrs_dict),
                                         ...
                                         help_text = _('Check to be hidden, otherwise displayed on site.'),
                                         required=False)
    hide_email = forms.BooleanField(widget=forms.CheckboxInput(attrs=attrs_dict),
                                         label = _('Hide Email Address?'),
                                         help_text = _('Check to be hidden, otherwise displayed on site.'),
                                         required = False)
    payment_option = forms.CharField(widget=forms.RadioSelect(choices=PAYMENT_OPTIONS),
                                     label = _("Payment Option"),
                                     required = True,
                                     error_messages={'required': _("You must select a payment option")})
    tos = forms.BooleanField(widget=forms.CheckboxInput(attrs=attrs_dict),
                             label=_(u'I have read and agree to the Terms of Service'),
                             error_messages={'required': _("You must agree to the terms to register")})
